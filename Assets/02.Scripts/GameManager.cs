...
public class GameManager : MonoBehaviour
{
    ...
    private HelixAttack LongRangeAttack4Script;
    ...
    private void UpdateWeaponStatus()
    {
        ...
        if (longRangeAttack4_Level == 0)
        {
            if (LongRangeAttack4Script != null)
            {
                LongRangeAttack4Script.enabled = false;
            }
        }
        else
        {
            if (LongRangeAttack4Script != null)
            {
                LongRangeAttack4Script.enabled = true;
            }
        }
    ...
    // 헬릭스
    private void UpdateLongRangeWeapon4Level()
    {
        HelixAttack longRangeAttack4 = player.GetComponent<HelixAttack>();
        longRangeAttack4.weaponLevel = longRangeAttack4_Level;  // 레벨 반영
        longRangeAttack4.UpdateFireRate(); // 발사 속도 업데이트
        longRangeAttack4.FireBullets();
    }
    #region 무기 변경
    // WeaponChange 메서드
