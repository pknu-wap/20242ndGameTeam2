...
{
    public Button ChangeButton;            // 변경 버튼
    ...
    private void Start()
    {
        cooldownText = ChangeButton.GetComponentInChildren<TextMeshProUGUI>(); // 자식 오브젝트에서 TMP 텍스트 찾기
        currentCooldown = cooldownTime;  // 쿨타임 초기화
        cooldownText.text = "";          // 초기에는 텍스트 비움

        ChangeButton.onClick.AddListener(ChangeButtonClick); // 버튼 클릭 리스너 추가
    }
    ...
    public void ChangeButtonClick()
    {
        if (!isCooldown)
        {
            ...
            if(GameManager.isMelee)
            {
                GameManager.isMelee = false;
            }
            else
            {
                GameManager.isMelee = true;
            }
        }
    }
